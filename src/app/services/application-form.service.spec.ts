import { TestBed } from '@angular/core/testing';
import { provideHttpClient } from '@angular/common/http';
import { provideHttpClientTesting } from '@angular/common/http/testing';
import { ApplicationFormService } from './application-form.service';

describe('ApplicationFormService', () => {
  let service: ApplicationFormService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [ApplicationFormService, provideHttpClient(), provideHttpClientTesting()],
    });
    service = TestBed.inject(ApplicationFormService);
  });

  it('continues from the intended sequence when the list includes a later outlier cluster', () => {
    (service as any).applicationRecords.set([
      { EmployeeCode: 'Emp-00000254' },
      { EmployeeCode: 'Emp-00003312' },
      { EmployeeCode: 'Emp-00003313' },
      { EmployeeCode: 'Emp-00003314' },
    ]);

    expect(service.getNextEmployeeCode()).toBe(255);
  });

  it('normalizes the legacy employee code to the required format', () => {
    expect(service.normalizeEmployeeCodeValue('Emp-00003283')).toBe('Emp-00000254');
  });

  it('remaps the outlier employee codes to the intended sequence', () => {
    expect(service.normalizeEmployeeCodeValue('Emp-00003312')).toBe('Emp-00000255');
    expect(service.normalizeEmployeeCodeValue('Emp-00003313')).toBe('Emp-00000256');
    expect(service.normalizeEmployeeCodeValue('Emp-00003314')).toBe('Emp-00000257');
  });

  it('uses the total remaining leave from leave-management rows in the flat payload', () => {
    const detail = {
      personalInfo: {
        personName: 'Test User',
        firstName: 'Test',
        middleName: '',
        lastName: 'User',
        fatherOrHusbandName: '',
        gender: 'Male',
        maritalStatus: 'Single',
        dateOfBirth: '',
        nationality: '',
        religion: '',
        bloodGroup: '',
        nationalIdCardNo: '',
        incomeTaxNo: '',
        contactNumber: '',
        emergencyContactNumber: '',
        street: '',
        streetNo: '',
        buildingFloorRoom: '',
        city: '',
        state: '',
        country: '',
        zipCode: '',
        employmentNature: '',
        employmentCategory: '',
        employmentStatus: '',
        departmentInAhcp: '',
        designation: '',
        jobDescription: '',
        roleSalary: '',
        workGradeLevel: '',
        branchLocation: '',
        costCenter: '',
        reportingManager: '',
        remarks: '',
      },
      education: [],
      pastExperience: [],
      remuneration: {
        basicSalary: '',
        paymentMode: '',
        accountTitle: '',
        bankName: '',
        branchName: '',
        accountNo: '',
        accountType: '',
        effectiveDate: '',
        taxPercentage: '',
        dateOfJoining: '',
        advancePercentAllowed: '',
        overTimeApplicable: '',
        leaveType: '',
        leaveDays: '',
        leavesAvailed: '',
        remainingLeaves: '',
        totalLeaves: '',
        medicalAllowances: '',
        fuelAllowances: '',
        mobileAllowances: '',
        carAllowances: '',
        maximumLoanCapacity: '',
        maximumAdvanceCapacity: '',
        otherAllowances: '',
        allowancesApplicable: '',
        cashSalaryPercentage: '',
        eobiApplicable: '',
        socialSecurityApplicable: '',
        fuelLimit: '',
        leaveEligibilityCriteria: '',
      },
      leaveManagement: [
        { leaveType: 'Annual', leavesAllocated: '20', leavesAvailed: '10', remainingLeave: '10' },
        { leaveType: 'Sick', leavesAllocated: '10', leavesAvailed: '4', remainingLeave: '6' },
      ],
      hrSettings: {
        employeeMaster: '',
        salaryStructure: '',
        attendanceShiftManagement: '',
        leaveManagement: '',
        loanAdvancesForm: '',
        requestStatus: '',
      },
      loginDetails: {
        employeeCode: '',
        employeeName: '',
        userId: '',
        password: '',
        status: '1',
      },
      attachments: [],
      requisition: {
        copyExisting: false,
        reqId: '',
        internalJobTitle: '',
        hiringManager: '',
        recruiter: '',
        recruitmentCollaborator: '',
        requisitionAdministrator: '',
        recruitmentCoordinator: '',
        hrAdministrator: '',
        company: '',
        department: '',
        division: '',
        location: '',
        costCenter: '',
      },
    } as any;

    const payload = service.buildFlatEmployeeProfilePayload(detail);

    expect(payload.remainingLeaves).toBe(16);
    expect(payload.totalLeaves).toBe(16);
  });
});
